...
impl UffdHandler {
    ...
    pub fn from_unix_stream_remote(stream: &UnixStream) -> Self {
        let (body, file) = Self::get_mappings_and_file(stream);
        let mappings =
            serde_json::from_str::<Vec<GuestRegionUffdMapping>>(&body).unwrap_or_else(|_| {
                panic!("Cannot deserialize memory mappings. Received body: {body}")
        ...
        Self {
            mem_regions: mappings,
            page_size,
            backing_buffer: ptr::null(),
            uffd,
        }
    }

    pub fn file_offset_for_addr(&self, addr: *mut u8) -> (u64, u64) {
        let dst = (addr as usize & !(self.page_size - 1)) as u64;
        for region in self.mem_regions.iter() {
            if region.contains(dst) {
                let offset = dst - region.base_host_virt_addr;
                return (dst, region.offset + offset);
            }
        }
    ...
    fn populate_from_file(&self, region: &GuestRegionUffdMapping, dst: u64, len: usize) -> bool {
        ...
        self.copy_from_ptr(src as *const _, dst as *mut _, len)
    }

    pub fn copy_from_slice(&self, dst: u64, data: &[u8]) -> bool {
        self.copy_from_ptr(data.as_ptr().cast(), dst as *mut _, data.len())
    }

    fn copy_from_ptr(&self, src: *const libc::c_void, dst: *mut libc::c_void, len: usize) -> bool {
        unsafe {
            match self.uffd.copy(src, dst, len, true) {
...
impl Runtime {
    pub fn new(stream: UnixStream, backing_file: File) -> Self {
        ...
        Self {
            stream,
            backing: BackingMemory::File {
                _file: backing_file,
                memory: ret.cast(),
                size: backing_memory_size,
            },
            uffds: HashMap::default(),
        }
    }

    pub fn new_remote(stream: UnixStream) -> Self {
        Self {
            stream,
            backing: BackingMemory::Remote,
            uffds: HashMap::default(),
        }
    }
    ...
    pub fn run(&mut self, mut pf_event_dispatch: impl FnMut(&mut UffdHandler)) {
        let mut pollfds = vec![];
        ...
        loop {
            ...
            if nready == -1 {
                ...
                if err.raw_os_error() == Some(libc::EINTR) && shutdown_requested() {
                    break;
                }
                panic!("Could not poll for events: {err}")
            }
            ...
            for i in 0..pollfds.len() {
                ...
                if pollfds[i].revents & libc::POLLIN != 0 {
                    ...
                    if pollfds[i].fd == self.stream.as_raw_fd() {
                        ...
                        let handler = match &self.backing {
                            BackingMemory::File { memory, size, .. } => {
                                UffdHandler::from_unix_stream(&self.stream, *memory, *size)
                            }
                            BackingMemory::Remote => {
                                UffdHandler::from_unix_stream_remote(&self.stream)
                            }
                        };
                        pollfds.push(libc::pollfd {
