...
/// Selects which subset of snapshot device restore to benchmark.
#[derive(Clone, Copy, Debug)]
pub enum BenchmarkDeviceRestoreKind {
    /// Restore all devices through `DeviceManager::restore`.
    All,
    /// Restore only the balloon MMIO device state.
    Balloon,
    /// Restore only block MMIO device state.
    Block,
    /// Restore only net MMIO device state.
    Net,
    /// Restore only the vsock MMIO device state.
    Vsock,
    /// Restore only the entropy MMIO device state.
    Entropy,
    /// Restore only PMEM MMIO device state.
    Pmem,
    /// Restore only the virtio-mem MMIO device state.
    VirtioMem,
}
...
impl Vmm {
    ...
    /// Kick all virtio devices without resuming vCPUs.
    pub fn kick_virtio_devices_for_resume(&self) {
        self.device_manager.kick_virtio_devices();
    }

    /// Send resume to all paused vCPUs and wait for responses, without kicking devices.
    pub fn resume_vcpus_after_kick(&mut self) -> Result<(), VmmError> {
        self.vcpus_handles
            .iter_mut()
            .try_for_each(|handle| handle.send_event(VcpuEvent::Resume))
            .map_err(|_| VmmError::VcpuMessage)?;
...
fn benchmark_filter_mmio_state(
    state: &device_manager::persist::DeviceStates,
    kind: BenchmarkDeviceRestoreKind,
) -> device_manager::persist::DeviceStates {
    let mut filtered = device_manager::persist::DeviceStates::default();

    match kind {
        BenchmarkDeviceRestoreKind::All => state.clone(),
        BenchmarkDeviceRestoreKind::Balloon => {
            filtered.balloon_device = state.balloon_device.clone();
            filtered
        }
        BenchmarkDeviceRestoreKind::Block => {
            filtered.block_devices = state.block_devices.clone();
            filtered
        }
        BenchmarkDeviceRestoreKind::Net => {
            filtered.net_devices = state.net_devices.clone();
            filtered.mmds = state.mmds.clone();
            filtered
        }
        BenchmarkDeviceRestoreKind::Vsock => {
            filtered.vsock_device = state.vsock_device.clone();
            filtered
        }
        BenchmarkDeviceRestoreKind::Entropy => {
            filtered.entropy_device = state.entropy_device.clone();
            filtered
        }
        BenchmarkDeviceRestoreKind::Pmem => {
            filtered.pmem_devices = state.pmem_devices.clone();
            filtered
        }
        BenchmarkDeviceRestoreKind::VirtioMem => {
            filtered.memory_device = state.memory_device.clone();
            filtered
        }
    }
}

/// Restore snapshot devices for benchmarking from a prepared VM context.
///
/// `All` goes through the full `DeviceManager::restore` path. The other variants
/// route through `MMIODeviceManager::restore` with a filtered MMIO device state,
/// so a single device family can be measured in isolation.
pub fn restore_devices_for_benchmark(
    ...
    match kind {
        BenchmarkDeviceRestoreKind::All => {
            let _device_manager = device_manager::DeviceManager::restore(
                device_manager::DeviceRestoreArgs {
                    mem: vm.guest_memory(),
                    vm,
                    event_manager,
                    vcpus_exit_evt,
            ...
            .map_err(|err| format!("DeviceManager::restore failed: {err}"))?;
        }
        _ => {
            let filtered = benchmark_filter_mmio_state(&microvm_state.device_states.mmio_state, kind);
            let _mmio_devices = device_manager::mmio::MMIODeviceManager::restore(
                device_manager::persist::MMIODevManagerConstructorArgs {
                    mem: vm.guest_memory(),
                    vm,
                    event_manager,
                    vm_resources,
            ...
            .map_err(|err| format!("MMIODeviceManager::restore failed: {err}"))?;
        }
    }
