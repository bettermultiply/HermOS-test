...
def main():
    ...
    cfg = load_config("eager-local.json", ["snapshot_a", "snapshot_b", "snapshot_state", "firecracker", "firecracker_cwd", "work_dir"])
    count = int(cfg["count"])
    ...
    try:
        ...
        parallel_map(count, lambda i: load_snapshot(api_sock(cfg["work_dir"], i), cfg["snapshot_state"], cfg["snapshot_a"]))
