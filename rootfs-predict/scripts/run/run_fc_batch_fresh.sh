...
if [ "$#" -lt 2 ] || [ "$#" -gt 4 ]; then
    echo "usage: run_fc_batch_fresh.sh TASK_FILE REPEAT_COUNT [JOBS] [START_RUN]" >&2
...
run_task_group() {
    ...
    local RUN_ID_NUMBER
    local RUN_ID
    local SNAPSHOT_DIR="$ROOT/snapshots/$INSTANCE_ID"
    ...
    for i in $(seq 1 "$REPEAT_COUNT"); do
        ...
        if [ -d "$SNAPSHOT_DIR" ]; then
            printf '[%s] removing snapshot dir: %s\n' "$(date '+%Y-%m-%d %H:%M:%S')" "$SNAPSHOT_DIR" >&2
            rm -rf "$SNAPSHOT_DIR"
        fi
